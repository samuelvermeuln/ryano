...
#
# Por que existe: na spec ryvano-escola-spec, 8 tasks ficaram marcadas como
# pendentes por semanas depois de entregues. Um agente que confiasse naquele
# estado teria reimplementado trabalho que já existia. Rodar isto antes de
# commitar evita repetir o erro (mesmo script, mesma razão, que
# ryvano-jornada-escola/check-status.sh já usa).
#
# Uso:  bash .kiro/specs/ryvano-marketplace-planos-treino/check-status.sh
...
ids = set(re.findall(r'^## \[.\] (TM\d+b?) ', txt, re.M))
pat = re.findall(
    r'^## \[.\] (TM\d+b?) .+?\n\n\*\*Tipo:\*\*.+?\n\*\*Prioridade:\*\*.+?\n\*\*Dependencias:\*\* (.+?)\n',
    txt, re.M | re.S)
...
import re, sys
txt = open('task-list.md').read()
st = dict((m[1], m[0]) for m in re.findall(r'^## \[(.)\] (TM\d+b?) ', txt, re.M))
pat = re.findall(
    r'^## \[.\] (TM\d+b?) .+?\n\n\*\*Tipo:\*\*.+?\n\*\*Prioridade:\*\*.+?\n\*\*Dependencias:\*\* (.+?)\n',
    txt, re.M | re.S)
