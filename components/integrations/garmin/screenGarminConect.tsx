  ...
  return (
    <>
            ...
            <div className="theme-panel-neutral rounded-[20px] border px-4 py-4 text-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <StatusBadge tone={reconnectNotification.status === "SENT" ? "success" : "danger"}>
                  {reconnectNotification.status === "SENT" ? "WhatsApp enviado" : "Envio falhou"}
                </StatusBadge>
              </div>
              <p className="mt-2">Tentativa em: {formatDateTime(reconnectNotification.createdAt)}</p>
              <p>Origem: {reconnectNotification.reason === "admin" ? "reenvio manual do admin" : "aviso automático do sistema"}</p>
              {reconnectNotification.errorCode ? <p>Motivo: {reconnectNotification.errorCode}</p> : null}
            </div>
          ) : null}
      ...
      {modalOpen && typeof document !== "undefined"
        ? createPortal(
            <div className="fixed inset-0 z-[999] overflow-y-auto bg-black/60 p-2 sm:p-4 md:p-6">
